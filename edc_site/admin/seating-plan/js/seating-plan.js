...
(function(){
 ...
 function guestSort(a,b){
  var gA=a.group||'',gB=b.group||'';
  if(gA!==gB)return gA.localeCompare(gB);
  var lA=a.lastName||'',lB=b.lastName||'';
  if(lA!==lB)return lA.localeCompare(lB);
  return (a.firstName||'').localeCompare(b.firstName||'');
 }
 function unassignedGuests(){var ids=assignedIds();return plan.guests.filter(function(g){return !ids.has(g.id)}).sort(guestSort)}
 ...
 function renderGuests(){var gs=$('#guestSearch');if(!gs||!plan)return;var q=gs.value.toLowerCase(); var cf=$('#countryFilter').value,co=$('#companyFilter').value,gf=$('#groupFilter').value,vip=$('#vipFilter').checked; var guests=unassignedGuests().filter(function(g){return C.guestName(g).toLowerCase().includes(q)}).filter(function(g){return (!cf||g.country===cf)&&(!co||g.company===co)&&(!gf||g.group===gf)&&(!vip||g.vip)}).sort(guestSort); if(selectedGuestId&&!guests.some(function(g){return g.id===selectedGuestId}))selectedGuestId=''; $('#guestList').innerHTML=guests.map(function(g){return '<li draggable="true" data-guest="'+g.id+'" class="'+(g.id===selectedGuestId?'selected':'')+'"><strong>'+C.guestName(g)+'</strong><span>'+[g.company,g.country,g.vip?'VIP':''].filter(Boolean).join(' \u00b7 ')+'</span><small class="tap-hint">Tap, then tap a chair</small><button data-edit-guest="'+g.id+'">Edit</button><button data-remove-guest="'+g.id+'">\u00d7</button></li>';}).join(''); ['country','company','group'].forEach(function(f){var el=$('#'+f+'Filter'),cur=el.value; var vals=[...new Set(plan.guests.map(function(g){return g[f]}).filter(Boolean))].sort(); el.innerHTML='<option value="">All '+f+'s</option>'+vals.map(function(v){return '<option '+(v===cur?'selected':'')+'>'+v+'</option>';}).join('')})}
 function renderCanvas(){var canvas=$('#canvas');if(!canvas||!plan)return; canvas.style.transform='translate('+plan.viewport.x+'px,'+plan.viewport.y+'px) scale('+plan.viewport.scale+')'; var seatScale=plan.viewport.seatScale||1; canvas.style.setProperty('--seat-scale',seatScale); var sz=$('#seatZoom'); if(sz)sz.value=seatScale; var szv=$('#seatZoomVal'); if(szv)szv.textContent=Math.round(seatScale*100)+'%'; canvas.innerHTML=plan.tables.map(function(t){return tableHtml(t)}).join('')}
 function tableHtml(t){var chairs=''; for(var i=1;i<=t.seats;i++){var a=plan.assignments.find(function(x){return x.tableId===t.id&&x.seatNumber===i}),g=a&&plan.guests.find(function(x){return x.id===a.guestId}); var ang=(i-1)/Math.max(t.seats,1)*Math.PI*2-Math.PI/2,x=120+Math.cos(ang)*92,y=120+Math.sin(ang)*92; chairs+='<button class="chair '+(g?'occupied':'')+'" style="left:'+x+'px;top:'+y+'px" data-table="'+t.id+'" data-seat="'+i+'" draggable="'+(!!g)+'" '+(g?'data-guest="'+g.id+'" data-tooltip="Seat '+i+': '+C.guestName(g)+(g.group?' ('+g.group+')':'')+'"':'title="Seat '+i+'"')+'>'+(g?'<span class="seat-number-badge">'+i+'</span><span class="guest-fn">'+(g.firstName||'')+'</span><span class="guest-ln">'+(g.lastName||'')+'</span>':'<span class="seat-number-empty">'+i+'</span>')+'</button>'} return '<div class="table" data-table-box="'+t.id+'" style="left:'+t.x+'px;top:'+t.y+'px"><div class="table-core" data-table-details="'+t.id+'"><strong>'+(t.label||'Table '+t.number)+'</strong><span>'+plan.assignments.filter(function(a){return a.tableId===t.id}).length+'/'+t.seats+'</span></div>'+chairs+'</div>'}
 ...
 ['guestSearch','countryFilter','companyFilter','groupFilter','vipFilter'].forEach(function(id){var el=$('#'+id);if(el)el.addEventListener('input',renderGuests)});
  ...
  var fsBtn=$('#fullscreenBtn');if(fsBtn)fsBtn.onclick=function(){
   document.body.classList.toggle('fullscreen-mode');
   var isFs=document.body.classList.contains('fullscreen-mode');
   fsBtn.innerHTML=isFs?'Exit Fullscreen \u26f6':'Toggle Fullscreen \u26f6';
  };
 ...
 var at=$('#addTable');if(at)at.onclick=function(){var p=structuredClone(plan);p.tables.push(C.createTable(p.tables.length+1,p.event.defaultSeats,150+p.tables.length*60,140));push(C.touch(p))};
 var gt=$('#gridTables');if(gt)gt.onclick=function(){var p=structuredClone(plan);p.tables.forEach(function(t,i){t.x=80+(i%4)*300;t.y=80+Math.floor(i/4)*300});push(C.touch(p))};
 ...
 var fv=$('#fitView');if(fv)fv.onclick=function(){var p=structuredClone(plan);p.viewport={scale:.8,x:0,y:0,seatScale:p.viewport.seatScale||1};push(C.touch(p))};
 var sz=$('#seatZoom');if(sz){sz.oninput=function(){var val=this.value; var canvas=$('#canvas');if(canvas)canvas.style.setProperty('--seat-scale',val); var szv=$('#seatZoomVal');if(szv)szv.textContent=Math.round(val*100)+'%'};sz.onchange=function(){var p=structuredClone(plan);p.viewport.seatScale=+this.value;push(C.touch(p))}}
